#!/usr/bin/env python3
"""PUT R2-3AB (2026-07-17) — response: direcciones del BA (party_dirs) + pais_destino.

UN nodo target: "Resolver Mailing".jsCode ← espejo con DOS campos ADITIVOS
de response: party_dirs {ship_to, sold_to, notify} (address del BA vía
contacts_extracted — R2-3a) + pais_destino (del embed puertos — R2-3b).
Cero cambios de template/bloqueos. El front los pinta en destinatarios y
junto al puerto.
...
EXPECT_NODES   = 36
EXPECT_CREDS   = 24
EXPECT_VER_PRE = "cf496aef-d399-4275-a3e4-5e745ea058cb"
...
TARGETS = {
    "Resolver Mailing": ("jsCode", RESOLVER_SRC,
                         ["party_dirs", "R2-3a/3b", "pais_destino,"],
                         "party_dirs"),
}
...
json.dump(pre, open(SDK + "workflow_pre_r23ab.json", "w"), ensure_ascii=False, indent=1)
...
json.dump(body, open(SDK + f"workflow_put_r23ab{'_dryrun' if DRY else ''}.json", "w"), ensure_ascii=False, indent=1)
if DRY:
...
json.dump(post, open(SDK + "workflow_post_r23ab.json", "w"), ensure_ascii=False, indent=1)
...
print("\nVEREDICTO R2-3AB: OK (Iron Law PASS)")
print("SMOKE REAL: preview 118833340 → party_dirs con las 3 direcciones + pais_destino=Brasil.")
