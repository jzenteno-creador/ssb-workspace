#!/usr/bin/env python3
"""PUT Tanda F — fix #1: count de facturas en BL multi-orden.

MODIFICA el jsCode de 1 nodo EXISTENTE (no agrega nodos ni conexiones):
  1. 'Inyectar Factura'  ← sdk/code_inyectar_factura.js

Root cause: $('GDrive: Buscar Factura').all().length en modo Run-Once-For-Each-Item
agrega los runs de TODAS las órdenes del BL multi-orden (exec 28596: 14 órdenes → fcount=14
→ duplicate=true → "Hay 14 facturas" falso positivo en cada mail).

Fix quirúrgico: filtrar .all() por nombre de archivo que contenga la orden del ítem actual
(extraída con orderFromName(u.name)), deduplicado por file id.

Iron Law: 49 nodos pre y post / active / drift SOLO en parameters.jsCode del target
(todos los demás campos byte-idénticos) / conexiones byte-idénticas / 15 refs de creds.
Aborta si versionId pre != fc45e29e-346c-4f44-9cba-fc7ef11c6941 (post Tanda BULK).
Auto-rollback.

Post-PUT: ciclo deactivate→activate (re-registra el polling trigger).

USO:
  python3 put_tanda_f.py --dry-run   # GET fresco, arma body, valida, NO hace PUT
  python3 put_tanda_f.py             # PUT real (solo con OK de John)
"""
...
EXPECT_NODES   = 49
EXPECT_CREDS   = 15
EXPECT_VER_PRE = "fc45e29e-346c-4f44-9cba-fc7ef11c6941"   # post Tanda BULK

TARGETS = {
    "Inyectar Factura": SDK + "code_inyectar_factura.js",
}
SANITY = {
    "Inyectar Factura": [
        "fix Tanda F",
...
json.dump(pre, open(SDK + "workflow_pre_tanda_f.json", "w"), ensure_ascii=False, indent=1)
...
# ---------- [3] drift-check pre-PUT: byte-idéntico salvo jsCode del target ----------
...
print("[3] body OK: drift solo en jsCode del target, conexiones byte-idénticas")
json.dump(body, open(SDK + ("workflow_put_tanda_f_dryrun.json" if DRY else "workflow_put_tanda_f.json"), "w"),
          ensure_ascii=False, indent=1)

if DRY:
    print("\nVEREDICTO (dry-run): body válido — 49 nodos, 1 jsCode nuevo, resto intacto. NO se hizo PUT.")
    sys.exit(0)
...
json.dump(post, open(SDK + "workflow_post_tanda_f.json", "w"), ensure_ascii=False, indent=1)
...
print("\nVEREDICTO Tanda F: OK (Iron Law PASS — 1 jsCode actualizado, resto byte-idéntico, trigger re-registrado)")
