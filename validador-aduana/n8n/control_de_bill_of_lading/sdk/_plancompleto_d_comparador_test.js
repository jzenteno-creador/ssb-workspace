...
const SRC_OLD = execSync('git show HEAD:validador-aduana/n8n/control_de_bill_of_lading/sdk/_comparador.js',
  { cwd: __dirname, encoding: 'utf8', maxBuffer: 1024 * 1024 * 8 });
const NEW = loadComparador(SRC_NEW);
const OLD = loadComparador(SRC_OLD);
...
// 1) ARICA/TACNA (POD Chile, destino final Perú en Aduana+Booking+Factura)
{
  ...
  const rOld = OLD.buildComparison(mkDoc());
  const eNew = destinoEntry(rNew), eOld = destinoEntry(rOld);
  ok(!!eNew && !!eOld, 'entry Destino presente en ambas versiones', JSON.stringify({ eNew: !!eNew, eOld: !!eOld }));
