#!/usr/bin/env python3
"""PUT-5d (tanda Factura): ajustes de render + 2 capturas raw en Code (sin tocar Parser/GATE/COMPARADOR).
Toca 3 nodos (Code):
  1. 'Inyectar Factura': items[].embalaje desde el raw ("25 KG Bags"), zip posicional.
  2. 'Inyectar links + order (Booking)': ba.bags_per_pallet desde el raw ("60 Bags on a Pallet") → derivar pallets en el mail.
  3. 'code  - plantilla HTML': Sold To ARRIBA de Consignee/Ship To; fila "Shipper / Exportador" unificada (BL ⇔ FC,
     elimina la duplicación Shipper+Exportador); desglose a la DERECHA del goods crudo (layout 2 columnas);
     embalaje BL (derivado "Bags" del goods) + FC por ítem; Booking en MONO con bolsas/pallets/neto/bruto poblados.
NO toca Parser Factura/Booking/Aduana, prompts, COMPARADOR ni conexiones/creds.
...
TARGETS = {N_FACTURA, N_BOOKING, N_PLANTILLA}
EXPECT_NODES = 45
EXPECT_CREDS = 14
...
SANITY = {
    N_FACTURA:   ["it.embalaje", "embalaje por ítem desde el raw"],
    N_BOOKING:   ["ba.bags_per_pallet", "Bags on a Pallet"],
    N_PLANTILLA: ["Shipper / Exportador", "soldToRowHtml", "blEmbalajeDoc", "bkBolsas", 'role="presentation"'],
}
...
json.dump(pre, open(SDK+"workflow_pre_render_5d.json", "w"), ensure_ascii=False, indent=1)
...
json.dump(body, open(SDK+"workflow_put_render_5d.json", "w"), ensure_ascii=False, indent=1)
...
json.dump(post, open(SDK+"workflow_post_render_5d.json", "w"), ensure_ascii=False, indent=1)
...
print("\nVEREDICTO PUT-5d: OK (Iron Law PASS — 45 nodos, drift solo en 3 targets, 14 creds, conexiones intactas)")
