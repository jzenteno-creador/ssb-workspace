#!/usr/bin/env python3
"""PUT T6·2 (G.2, 2026-07-17) — workflow "Mailing Envío Documentación" kh6TORgRg9R1Shj1.

TRES nodos target (drift permitido EXACTAMENTE en ellos, nada más):
  1. "Resolver Mailing".parameters.jsCode ← sdk/code_mailing_resolver.js
     · template "Slim & Structured" de la guía de John (EN, email-safe estricto):
       KPI ETD+ATD+ETA+tránsito, Shipment/Incoterm/Freight (columnas T6·1),
       banderas emoji, checklist EN, FREE DAYS, SHIPPING LINE excluido (P·6),
       saludo genérico + empresa en asunto, cero data-cfemail/scripts.
     · días libres desde v_orden_freetime con regla hub P·5 (fila SIN sufijo).
     · contratos response/route/root INTACTOS (verificado offline 31 asserts:
       node sdk/_t6_resolver_test.cjs).
  2. "GET puertos pais".parameters.url ← select ampliado con embed paises
     (pais_iso FK → nombre_en + flag_emoji). El campo `pais` se CONSERVA:
     "GET naviera destino" lo sigue leyendo con first().json.pais.
  3. "GET detention".parameters.url ← repunte a v_orden_freetime keyed por
     order_number (resolutor T4) — mueren los mapas inline DET_SUPPLIER/
     DET_COUNTRY. Puede devolver 2 filas (variantes hub); el Resolver elige.
...
EXPECT_NODES   = 33
EXPECT_CREDS   = 21
EXPECT_VER_PRE = "bce090d2-0eb9-4925-a4d7-cc36abc0328b"
...
TARGETS = {
    "Resolver Mailing": ("jsCode", RESOLVER_SRC,
                         ["T6·2", "Shipping Documents · Order",
                          "FREE DAYS AT DESTINATION", "v_orden_freetime",
                          "Dear Customer", "HUB|ONLY"],
                         "SHIPPING DOCUMENTS"),
    "GET puertos pais": ("url", URL_PUERTOS,
                         ["paises(nombre_en,flag_emoji)", "select=pais,"],
                         "flag_emoji"),
    "GET detention":    ("url", URL_FREETIME,
                         ["v_orden_freetime", "order_number=eq."],
                         "v_orden_freetime"),
}
...
json.dump(pre, open(SDK + "workflow_pre_t6_template.json", "w"), ensure_ascii=False, indent=1)
...
json.dump(body, open(SDK + f"workflow_put_t6_template{'_dryrun' if DRY else ''}.json", "w"), ensure_ascii=False, indent=1)
if DRY:
    print("\nVEREDICTO [DRY-RUN]: LIMPIO.")
    print(f"  versionId LIVE == pin {EXPECT_VER_PRE} : PASS")
    print(f"  node count {EXPECT_NODES} sin cambio (3 edits in-place) : PASS")
...
json.dump(post, open(SDK + "workflow_post_t6_template.json", "w"), ensure_ascii=False, indent=1)
...
print(f"  node_count==33 : {'PASS' if len(post['nodes'])==EXPECT_NODES else 'FAIL'}")
print(f"  cred-refs==21  : {'PASS' if len(cred_ids(post))==EXPECT_CREDS else 'FAIL'}")
...
print("\nVEREDICTO T6·2: OK (Iron Law PASS)")
print("SMOKE REAL (la hace Claude, sin mail): POST preview 118833340 al webhook → body nuevo + dias_libres 21/35/10.")
