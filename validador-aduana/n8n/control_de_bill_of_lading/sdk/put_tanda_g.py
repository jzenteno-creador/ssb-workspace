#!/usr/bin/env python3
"""PUT Tanda G — 4 fixes quirúrgicos en _comparador.js.

G2: eqEmb — Big Bag(s)/BigBag(s) ≡ Bag(s) (solo esa familia; Bulk/otros intactos).
G3: Split nombre_difiere/nombre_dif_info — regla del ancla: lead confirmado por ≥2 docs
    + distinción core-match vs mono-attach para BA.
G4: Gate /\d/ en bookingBA → NODATA informativo si no tiene dígitos (pre-reserva 'X').
G6: Dynamic refN en triage — primera fuente presente NO listada en nombre_difiere.
...
Aborta si versionId pre != b83905c7-70e7-44d1-8ca7-5fd493441591 (post Tanda F).
Auto-rollback.

Post-PUT: ciclo deactivate→activate (re-registra el polling trigger).

USO:
  python3 put_tanda_g.py --dry-run   # GET fresco, arma body, valida, NO hace PUT
  python3 put_tanda_g.py             # PUT real (solo con OK de John)
"""
...
EXPECT_NODES   = 49
EXPECT_CREDS   = 15
EXPECT_VER_PRE = "b83905c7-70e7-44d1-8ca7-5fd493441591"   # post Tanda F
...
SANITY = {
    "COMPARADOR - BL vs Aduana vs Booking": [
        ...
        "nombre_dif_info",
        "leadDocCount",
        "verificar con el despachante si corresponde",
        # G4
        "_bookingHasDigit",
        "pre-reserva",
        # G6
...
json.dump(pre, open(SDK + "workflow_pre_tanda_g.json", "w"), ensure_ascii=False, indent=1)
...
json.dump(body, open(SDK + ("workflow_put_tanda_g_dryrun.json" if DRY else "workflow_put_tanda_g.json"), "w"),
...
json.dump(post, open(SDK + "workflow_post_tanda_g.json", "w"), ensure_ascii=False, indent=1)
...
print("\nVEREDICTO Tanda G: OK (Iron Law PASS — 1 jsCode actualizado, resto byte-idéntico, trigger re-registrado)")
