#!/usr/bin/env python3
"""PUT-5c (tanda Factura): reorg render según mockup + 3 capturas raw en Code (sin tocar Parser/GATE/COMPARADOR).
Toca 3 nodos (Code):
  1. 'Inyectar Factura': factura_extract.exporter (dirección completa del emisor desde raw) + items[].bags_per_pallet
     y items[].pallets (= bolsas / N-bags-on-a-pallet, sin hardcodear 60).
  2. 'Inyectar pe + source_link' (Aduana): contenedores[].bultos por contenedor desde el raw (suma por grade = pallets).
  3. 'code  - plantilla HTML': cuadro 4 columnas con 'abrir' en headers (sin sección Documentos suelta ni fechas BA),
     Incoterm Place / Permiso / Consignee=ShipTo / Sold To / Exportador integrados, Vessel/Voyage unificado,
     Brazil estandarizado, fila Grado retirada, nota Notify, flete a columna 'Valor en factura'+Estado, goods crudo,
     desglose por producto (encabezado = nombre completo del BL; Aduana pallets, FC pallets/embalaje).
NO toca Parser Factura/Aduana, prompts, COMPARADOR ni conexiones/creds.
...
N_FACTURA   = "Inyectar Factura"
N_ADUANA    = "Inyectar pe + source_link"
N_PLANTILLA = "code  - plantilla HTML"
TARGETS = {N_FACTURA, N_ADUANA, N_PLANTILLA}
EXPECT_NODES = 45
EXPECT_CREDS = 14

FILES = {
    N_FACTURA:   SDK + "code_inyectar_factura.js",
    N_ADUANA:    SDK + "code_inyectar_pe_source_link.js",
    N_PLANTILLA: SDK + "_plantilla_html.js",
}
SANITY = {
    N_FACTURA:   ["exporterFromRaw", "bags_per_pallet"],
    N_ADUANA:    ["parseBultosAduana", "c.bultos"],
    N_PLANTILLA: ["Valor en factura", "aduBultosByGrade", "Vessel / Voyage", "abrir</a>"],
}
...
json.dump(pre, open(SDK+"workflow_pre_render_5c.json", "w"), ensure_ascii=False, indent=1)
...
json.dump(body, open(SDK+"workflow_put_render_5c.json", "w"), ensure_ascii=False, indent=1)
...
json.dump(post, open(SDK+"workflow_post_render_5c.json", "w"), ensure_ascii=False, indent=1)
...
print(f"  drift SOLO 3 targets: {'PASS' if not unexpected_drift else 'FAIL '+str(unexpected_drift)}  ({sorted(set(target_ok))})")
...
print("\nVEREDICTO PUT-5c: OK (Iron Law PASS — 45 nodos, drift solo en 3 targets, 14 creds, conexiones intactas)")
