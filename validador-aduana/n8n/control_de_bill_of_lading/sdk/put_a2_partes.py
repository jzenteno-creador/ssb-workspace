#!/usr/bin/env python3
"""PUT A2 (2026-07-18, backlog Log-In R1-R12 · Workstream A · PUT-M2) — bloque
"Partes" (Sold-to/Ship-to/Notify) en el espejo del nodo "Resolver Mailing".

UN nodo target: "Resolver Mailing".jsCode ← espejo con el bloque nuevo del
17-07 SPEC (John): sección "PARTIES"/"PARTES" en el cuerpo del mail, insertada
antes de PRODUCT (agrupa identidad del embarque, después de SHIPMENT DETAILS):
  - Sold-to / Ship-to: nombre (mailing_orders, fallback contacts_extracted) +
    dirección (MISMA fuente que response.party_dirs — ya existía, sin
    duplicar lógica).
  - Notify: cadena de fallbacks que NUNCA degrada en silencio —
    1º bl_extract.notify (columna real de bl_controls, expuesta sin select=
    por "GET control BL (latest)"; multilínea, LÍNEA 1 = nombre; Maersk ya
    resuelve "SAME AS CONSIGNEE" río arriba en code_inyectar_metadata_maersk.js,
    LOG-IN nunca la usa) → 2º notify_name (mailing_orders) → 3º
    contacts_extracted.notify.name → nada de eso: marca visible
    "⚠ SIN NOTIFY"/"⚠ NOTIFY NOT ON FILE"/"⚠ SEM NOTIFY" (PACKS L, un idioma
    cada uno) — jamás un "—" mudo.
  - PACKS L completos (secPa/lSoldTo/lShipTo/lNotify/noNotify) en los 3
    idiomas — mismo sistema R2·D ya vigente (MAIL_LANG en/es/pt).
No toca: response.notify (la CLAVE del directorio de contactos, 2 niveles sin
bl_extract — otro consumidor, intacta), el control de discrepancia de notify
del comparador (RFORNE on-behalf, fix operativo fuera de este archivo), el
subject (NI UN BYTE — solo se tocó body_html), ni el bloque Log-In (A1,
LOGIN_HEAD en portugués) que sigue exactamente igual — se verifica su
presencia como marker de continuidad.

Iron Law: 36 nodos, 24 cred-refs (Gmail wWZzmUj5MQLrECH0 intacto — NO relink),
active=true, versionId pre == pin 990a5fc4-ef3b-481d-8637-75480f72ce66 (pin
post-A1, vivo — NO reverificado por este script contra n8n antes de correr: es
un dato de entrada, el precheck [0] es quien lo contrasta contra el GET real),
nodos NO-target byte-idénticos, connections pre↔post BYTE-IDÉNTICAS: mismo
patrón heredado de put_r2_3ab_resolver.py / put_a1_login_head.py (drift-check
+ Iron Law vía igualdad estructural de dict/list de Python — recursiva,
independiente del orden de claves, idéntica en poder de detección a
json.dumps(..., sort_keys=True); las listas conservan orden en ambos métodos,
que es lo correcto). auto-rollback con el body pre si cualquier check falla,
...
EXPECT_NODES   = 36
EXPECT_CREDS   = 24
EXPECT_VER_PRE = "990a5fc4-ef3b-481d-8637-75480f72ce66"
GMAIL_CRED     = "wWZzmUj5MQLrECH0"

# string único del bloque nuevo (marca del tier 4 de la cadena notify, texto
# ES EXACTO firmado por John en el SPEC 17-07) — sirve de marker (¿el espejo
# cargado es el árbol completo con A2?) Y de live_guard (¿ya se aplicó antes?).
PARTES_MARK = "⚠ SIN NOTIFY"
# A1 debe seguir intacto (LOGIN_HEAD ya traducido a portugués) — continuidad.
LOGIN_HEAD_PT_FRAG = "Favor entrar em contato"

RESOLVER_SRC = open(SDK + "code_mailing_resolver.js", encoding="utf-8").read()
TARGETS = {
    "Resolver Mailing": ("jsCode", RESOLVER_SRC,
                         [PARTES_MARK, LOGIN_HEAD_PT_FRAG, "party_dirs"],
                         PARTES_MARK),
}
...
json.dump(pre, open(SDK + "workflow_pre_a2partes.json", "w"), ensure_ascii=False, indent=1)
...
for tgt, (param, _v, _m, live_guard) in TARGETS.items():
    ...
    if LOGIN_HEAD_PT_FRAG not in str(node["parameters"].get(param, "")):
        sys.exit(f"ABORT: {tgt!r} LIVE sin {LOGIN_HEAD_PT_FRAG!r} — A1 no está aplicado, HALT (este script asume A1 ya en prod)")
...
# connections byte-idénticas — heredado de put_a1_login_head.py, NO duplicado
...
json.dump(body, open(SDK + f"workflow_put_a2partes{'_dryrun' if DRY else ''}.json", "w"), ensure_ascii=False, indent=1)
if DRY:
    ...
    print(f"  A1 (LOGIN_HEAD PT) presente en LIVE pre : PASS")
...
json.dump(post, open(SDK + "workflow_post_a2partes.json", "w"), ensure_ascii=False, indent=1)
...
print("\n===== IRON LAW (A2 · bloque Partes, 1 target) =====")
...
print("\nVEREDICTO A2 (bloque Partes): OK (Iron Law PASS)")
print("SMOKE REAL: preview de cualquier orden asentada en mailing_orders → sección")
print("PARTIES/PARTES en el cuerpo, antes de PRODUCT, con Sold-to/Ship-to (nombre +")
print("dirección si hay) y Notify por la cadena de fallbacks (o la marca visible")
print("si no hay dato en ningún nivel); LOGIN_HEAD (A1) y el resto del template sin cambios.")
