#!/usr/bin/env python3
"""PUT T6·3 (G.2, 2026-07-17) — workflow "Mailing Envío Documentación" kh6TORgRg9R1Shj1.

Checklist ATTACHED DOCUMENTS con fuente de verdad en documentos_orden (D.2/T5):

  1. NUEVO nodo "GET documentos_orden" (httpRequest, cred supabaseApi, best-effort:
     onError continue + alwaysOutputData) insertado en la cadena lineal:
        Buscar SEG → GET documentos_orden → Resolver Mailing
     (33 → 34 nodos; 21 → 22 cred-refs; ÚNICO cambio de conexiones permitido.)
  2. "Resolver Mailing".parameters.jsCode ← sdk/code_mailing_resolver.js
     · adjuntos con ✓ + docs registrados no adjuntos "(to follow)" — solo tipos
       client-facing (factura/packing/PE/CRT), PE gateado por order_kind;
       booking ZCB1 y "otros" jamás. response.attachments.to_follow aditivo.
     · verificado offline 36 asserts: node sdk/_t6_resolver_test.cjs.
...
EXPECT_NODES_PRE  = 33
EXPECT_NODES_POST = 34
EXPECT_CREDS_PRE  = 21
EXPECT_CREDS_POST = 22
EXPECT_VER_PRE    = "f997deff-da8c-40ce-8b09-c94d9333fa94"
GMAIL_CRED        = "wWZzmUj5MQLrECH0"
SUPA_CRED         = {"id": "aQoShf0TVYyf2lrt", "name": "Supabase account ssb workspace"}

NEW_NAME = "GET documentos_orden"
NEW_NODE = {
    "id": "t6-get-docs-orden-01",
    "name": NEW_NAME,
    ...
    "parameters": {
        "url": ("={{ 'https://xkppkzfxgtfsmfooozsm.supabase.co/rest/v1/documentos_orden"
                "?select=tipo,file_name&order_number=eq.'"
...
PREV_NODE = "Buscar SEG"       # el nodo que hoy conecta directo al Resolver
NEXT_NODE = "Resolver Mailing"

RESOLVER_SRC = open(SDK + "code_mailing_resolver.js", encoding="utf-8").read()
for mk in ["T6·3", "(to follow)", "GET documentos_orden", "docs_to_follow", "onlyTrade"]:
...
json.dump(pre, open(SDK + "workflow_pre_t6_checklist.json", "w"), ensure_ascii=False, indent=1)
...
if "(to follow)" in res_pre["parameters"].get("jsCode", ""):
    sys.exit("ABORT: resolver LIVE ya contiene T6·3 (¿corrida previa?)")
...
json.dump(body, open(SDK + f"workflow_put_t6_checklist{'_dryrun' if DRY else ''}.json", "w"), ensure_ascii=False, indent=1)
if DRY:
...
json.dump(post, open(SDK + "workflow_post_t6_checklist.json", "w"), ensure_ascii=False, indent=1)
...
print("\nVEREDICTO T6·3: OK (Iron Law PASS)")
print("SMOKE REAL (Claude, sin mail): preview 118833340 → attachments.to_follow presente")
print("  + n8n-cli execution: nodo 'GET documentos_orden' devolvió las filas del registro.")
