#!/usr/bin/env python3
"""PUT A2-FIX — created_at refresca en cada control (GO John 2026-07-17).

Transform sobre WVt6gvghL2nFVbt6 (pin pre: 7cf87074-…, 69 nodos — 1 edit IN-PLACE):
  T1: "Armar fila Control BL" ← espejo code_armar_fila_control_bl.js con created_at
      en el payload del upsert → el re-control refresca bl_controlado_at en la UI
      (root cause del 'no se actualizó la web' del reproceso) y habilita el poll
      de A.2-front. Marker: 'A2-FIX (2026-07-17'.
...
USO:
  python3 put_a2fix_created_at.py --dry-run [snapshot.json]
  python3 put_a2fix_created_at.py --apply
"""
...
EXPECT_VER_PRE    = "7cf87074-3e5b-4a1c-bf99-b1219b2c5a1a"
EXPECT_NODES_PRE  = 69
EXPECT_NODES_POST = 69
SUPA_URL = "https://xkppkzfxgtfsmfooozsm.supabase.co/rest/v1"

N_ARMAR = "Armar fila Control BL"
TARGETS = {N_ARMAR}
...
def apply_transforms(pre):
    ...
    code = open(SDK + "code_armar_fila_control_bl.js", encoding="utf-8").read()
    if "A2-FIX (2026-07-17" not in code:
        sys.exit("ABORT: el espejo no es la versión A2-FIX (falta marker)")
    if "PLAN1-FIX6" not in code or "email_sent:" in code:
        sys.exit("ABORT: el espejo perdió los invariantes de PLAN1-FIX6")
    if "A2-FIX (2026-07-17" in by_name[N_ARMAR]["parameters"]["jsCode"]:
...
def verify(pre, nodes, conns, label):
    ...
    if "A2-FIX (2026-07-17" not in post_by[N_ARMAR]["parameters"]["jsCode"]:
...
def main():
    ...
    if mode == "--dry-run":
        ...
        out = {"name": pre["name"], "nodes": nodes, "connections": conns,
               "settings": {"executionOrder": "v1"}}
        json.dump(out, open(SDK + "workflow_a2fix_preview.json", "w"), ensure_ascii=False, indent=1)
        print("preview →", SDK + "workflow_a2fix_preview.json")
    ...
    json.dump(pre, open(SDK + "workflow_pre_a2fix.json", "w"), ensure_ascii=False, indent=1)
    ...
    json.dump(post, open(SDK + "workflow_post_a2fix.json", "w"), ensure_ascii=False, indent=1)
    ...
    print("VERIFICACIÓN REAL (la hace Claude): reprocesar 118833340 → created_at fresco en bl_controls.")
