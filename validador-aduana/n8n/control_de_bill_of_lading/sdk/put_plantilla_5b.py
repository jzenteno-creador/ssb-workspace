#!/usr/bin/env python3
"""PUT-5b (tanda Factura): render plantilla + 2 capturas de texto crudo (sin tocar prompts ni COMPARADOR).
Toca 3 nodos (Code):
  1. 'Inyectar metadata (LOG-IN)': captura login_extract.goods_block_raw (bloque DESCRIPTION OF PACKAGES
     AND GOODS del BL, tal cual, desde u.text) — el raw se pierde río abajo, por eso se captura acá.
  2. 'Inyectar Factura': captura items[].product_code (PA/NCM por producto) del raw FC.
  3. 'code  - plantilla HTML': cuadro 4 columnas (BL/Aduana/BA/FACTURA), Sold/Ship/Exportador integrados,
     permiso comparado BL/Aduana/FC, retiro fila Grado, nota Notify interpretable, sección Productos
     (desglose por producto + bloque goods crudo en <pre>), flete FC (total + por contenedor vs BL).
NO toca COMPARADOR, NO toca prompts/schemas, NO toca conexiones/creds.
...
TARGETS = {N_LOGIN, N_FACTURA, N_PLANTILLA}
EXPECT_NODES = 45
EXPECT_CREDS = 14

FILES = {
    N_LOGIN:    SDK + "code_inyectar_metadata_login.js",
    N_FACTURA:  SDK + "code_inyectar_factura.js",
    N_PLANTILLA: SDK + "_plantilla_html.js",
}
SANITY = {
    N_LOGIN:    ["goods_block_raw"],
    N_FACTURA:  ["product_code"],
    N_PLANTILLA: ["goodsRaw", "rowFacturaFlete", "<th>FACTURA</th>"],
}
...
json.dump(pre, open(SDK+"workflow_pre_plantilla_5b.json", "w"), ensure_ascii=False, indent=1)
...
# guard: NO debe quedar la fila legacy en plantilla NI tocarse el COMPARADOR (no está en targets)
if "COMPARADOR" in " ".join(TARGETS): pass

body = {"name": pre["name"], "nodes": nodes, "connections": pre["connections"], "settings": {"executionOrder": "v1"}}
json.dump(body, open(SDK+"workflow_put_plantilla_5b.json", "w"), ensure_ascii=False, indent=1)
...
json.dump(post, open(SDK+"workflow_post_plantilla_5b.json", "w"), ensure_ascii=False, indent=1)
...
print("\nVEREDICTO PUT-5b: OK (Iron Law PASS — 45 nodos, drift solo en 3 targets, 14 creds, conexiones intactas)")
