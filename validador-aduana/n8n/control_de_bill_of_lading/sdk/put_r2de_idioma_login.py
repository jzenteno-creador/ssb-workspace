#!/usr/bin/env python3
"""PUT R2·DE (2026-07-17) — mail: IDIOMA por destino + contactos LOG-IN al pie.

UN nodo target: "Resolver Mailing".jsCode ← espejo con:
  R2·D — packs de idioma completos (en/es/pt) elegidos por país DESTINO:
    Brasil→pt · Latinoamérica hispana→es · resto (incl. USA)→en. Asunto,
    saludo, secciones, checklist, FREE DAYS, cierre y confidencial traducidos;
    valores de comercio (Prepaid/Collect, DRY/REEFER, Incoterm) quedan EN.
    response.mail_lang aditivo. TEST banner sigue en ES (interno).
  R2·E — bloque contactos LOG-IN al pie SOLO si carrier=LOG-IN, texto TEXTUAL
    de John verbatim (portugués oficial de la naviera), emails clickeables.
    El bloque mailing_naviera_destino SALIÓ del template (P·6 reemplazado;
    el nodo GET queda sin consumo hasta las casillas de Maersk/Hapag).
  Verificado offline: 51 asserts (fixture Brasil→pt, Perú→es, USA→en,
    MAERSK sin bloque, LOG-IN con contactos verbatim).
...
EXPECT_NODES   = 36
EXPECT_CREDS   = 24
EXPECT_VER_PRE = "a624b3d5-6e88-475c-a93a-f0543f54b7cb"
...
TARGETS = {
    "Resolver Mailing": ("jsCode", RESOLVER_SRC,
                         ["MAIL_LANG", "PACKS", "LOGIN_LINES", "loginlogistica", "R2·E"],
                         "MAIL_LANG"),
}
...
json.dump(pre, open(SDK + "workflow_pre_r2de.json", "w"), ensure_ascii=False, indent=1)
...
json.dump(body, open(SDK + f"workflow_put_r2de{'_dryrun' if DRY else ''}.json", "w"), ensure_ascii=False, indent=1)
if DRY:
...
json.dump(post, open(SDK + "workflow_post_r2de.json", "w"), ensure_ascii=False, indent=1)
...
print("\nVEREDICTO R2·DE: OK (Iron Law PASS)")
print("SMOKE REAL: preview 118833340 (BR) → mail en PT; 118762005 (PE) → ES; sin Log-In en MAERSK.")
