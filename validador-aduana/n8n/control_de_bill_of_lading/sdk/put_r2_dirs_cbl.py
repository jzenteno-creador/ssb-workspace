#!/usr/bin/env python3
"""PUT R2-DIRS — Armar fila Mailing suma DIRECCIONES del BA a contacts_extracted (R2-3a).

Transform sobre WVt6gvghL2nFVbt6 (pin pre: 051731bd, 73 nodos — 1 edit IN-PLACE):
  T1: "Armar fila Mailing" ← espejo con address (address_str del parser
      Booking — la extracción YA EXISTÍA, esto es plomería) en consignee/
      sold_to/notify de contacts_extracted. Marker: 'R2-3a (2026-07-17'.
...
USO:
  python3 put_r2_dirs_cbl.py --dry-run [snapshot.json]
  python3 put_r2_dirs_cbl.py --apply
"""
...
EXPECT_VER_PRE    = "051731bd-cf9e-460c-abf9-21738ef21907"
EXPECT_NODES_PRE  = 73
EXPECT_NODES_POST = 73
...
def apply_transforms(pre):
    ...
    if "R2-3a (2026-07-17" not in code:
    ...
    if "R2-3a (2026-07-17" in by_name[N_ARMAR]["parameters"]["jsCode"]:
...
def verify(pre, nodes, conns, label):
    ...
    if "R2-3a (2026-07-17" not in post_by[N_ARMAR]["parameters"]["jsCode"]:
...
def main():
    ...
    if mode == "--dry-run":
        ...
        out = {"name": pre["name"], "nodes": nodes, "connections": conns,
               "settings": {"executionOrder": "v1"}}
        json.dump(out, open(SDK + "workflow_r2dirs_preview.json", "w"), ensure_ascii=False, indent=1)
        print("preview →", SDK + "workflow_r2dirs_preview.json")
    ...
    json.dump(pre, open(SDK + "workflow_pre_r2dirs.json", "w"), ensure_ascii=False, indent=1)
    ...
    json.dump(post, open(SDK + "workflow_post_r2dirs.json", "w"), ensure_ascii=False, indent=1)
    ...
    print("VERIFICACIÓN: backfill cubre lo histórico; futuros asientos llevan address solos.")
