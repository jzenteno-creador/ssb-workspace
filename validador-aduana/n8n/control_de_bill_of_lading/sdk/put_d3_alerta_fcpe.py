#!/usr/bin/env python3
"""PUT D3-ALERTA (2026-07-17) — workflow "Mailing Envío Documentación" kh6TORgRg9R1Shj1.

Alerta FC-PE en el punto de envío (decisión John 17-07: AVISA, NO bloquea):

  1. NUEVO nodo "GET controles_factura_pe" (httpRequest best-effort) insertado:
        GET orden_productos → GET controles_factura_pe → Resolver Mailing
     (35 → 36 nodos; 23 → 24 cred-refs; ÚNICO cambio de conexiones permitido.)
  2. "Resolver Mailing".jsCode ← espejo con response.control_fcpe ADITIVO
     (overall_result + checks + pe_numero) — JAMÁS entra en block_reasons ni
     en el mail al cliente; el front lo pinta como warning.
...
EXPECT_CREDS_PRE  = 23
EXPECT_CREDS_POST = 24
EXPECT_VER_PRE    = "c5920781-4e6f-4368-b0e8-dfd48fcf55de"
GMAIL_CRED        = "wWZzmUj5MQLrECH0"
SUPA_CRED         = {"id": "aQoShf0TVYyf2lrt", "name": "Supabase account ssb workspace"}

NEW_NAME = "GET controles_factura_pe"
NEW_NODE = {
    "id": "d3-get-fcpe-01",
    "name": NEW_NAME,
    ...
    "parameters": {
        "url": ("={{ 'https://xkppkzfxgtfsmfooozsm.supabase.co/rest/v1/controles_factura_pe"
                "?select=overall_result,checks,pe_numero&limit=1&order_number=eq.'"
...
PREV_NODE = "GET orden_productos"  # el nodo que hoy conecta directo al Resolver
NEXT_NODE = "Resolver Mailing"

RESOLVER_SRC = open(SDK + "code_mailing_resolver.js", encoding="utf-8").read()
for mk in ["control_fcpe", "GET controles_factura_pe", "NO BLOQUEA", "AVISA"]:
...
json.dump(pre, open(SDK + "workflow_pre_d3alerta.json", "w"), ensure_ascii=False, indent=1)
...
if "control_fcpe" in res_pre["parameters"].get("jsCode", ""):
    sys.exit("ABORT: resolver LIVE ya contiene D3-ALERTA (¿corrida previa?)")
...
json.dump(body, open(SDK + f"workflow_put_d3alerta{'_dryrun' if DRY else ''}.json", "w"), ensure_ascii=False, indent=1)
if DRY:
...
json.dump(post, open(SDK + "workflow_post_d3alerta.json", "w"), ensure_ascii=False, indent=1)
...
print("\nVEREDICTO D3-ALERTA: OK (Iron Law PASS)")
print("SMOKE REAL: preview 118762005 (REVISAR) → control_fcpe.overall_result REVISAR y send NO bloqueado por FC-PE.")
