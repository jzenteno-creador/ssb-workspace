#!/usr/bin/env python3
"""PUT FLAGS-FIX (2026-07-17) — banderas del mail: emoji → img flagcdn (bug smoke John).

UN nodo target: "Resolver Mailing".jsCode ← espejo con flagImg(iso2, nombre):
imágenes flagcdn 24x18 keyed por pais_iso (mismo mecanismo que Detention/
Schedule en la app) — Windows degradaba los emoji al código crudo "BR".
Canal email: imágenes bloqueadas → alt = NOMBRE del país (presentable);
el nombre además ya viaja impreso bajo la ciudad. Cero emoji en el body.
...
EXPECT_NODES   = 36
EXPECT_CREDS   = 24
EXPECT_VER_PRE = "2d61baed-8d6e-402d-8a26-e126df20e62f"
...
TARGETS = {
    "Resolver Mailing": ("jsCode", RESOLVER_SRC,
                         ["flagcdn.com/24x18/", "flagImg", "NUNCA emoji"],
                         "flagcdn"),
}
...
json.dump(pre, open(SDK + "workflow_pre_flagsfix.json", "w"), ensure_ascii=False, indent=1)
...
json.dump(body, open(SDK + f"workflow_put_flagsfix{'_dryrun' if DRY else ''}.json", "w"), ensure_ascii=False, indent=1)
if DRY:
...
json.dump(post, open(SDK + "workflow_post_flagsfix.json", "w"), ensure_ascii=False, indent=1)
...
print("\nVEREDICTO FLAGS-FIX: OK (Iron Law PASS)")
print("SMOKE REAL: preview 118833340 → img flagcdn ar+br con alt de nombre, cero emoji.")
