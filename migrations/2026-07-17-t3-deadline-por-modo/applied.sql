-- ═══════════════════════════════════════════════════════════════════════════
-- T3·1 — v_operacion_estado: deadline_envio POR MODO + universe endurecido
-- (plan pedidos B.2, decisión CERRADA; §5 TANDA-BASE ahora FK-garantizado)
-- · universe = seguimiento_ordenes (superset garantizado por FKs de T4.a —
--   pre-flight en el apply: las 3 ramas UNION viejas aportan 0 filas)
-- · deadline_envio: terrestre = COALESCE(atd, despacho_at) + 1 día HÁBIL
--   (GI = inicia tránsito; vie→lun, sáb→lun); marítimo = atd + 4 (intacto)
-- · inicia_transito (columna NUEVA, al final): terrestre COALESCE(atd,
--   despacho_at); marítimo atd — para la columna renombrada del front (B.2)
-- · DECISIÓN: la alerta envio_vencido queda anclada a atd (mailing) como hoy —
--   migrarla al deadline por modo inundaría el badge con las 34 terrestres
--   legacy; se revisa con John en el front de T3·3
-- Canal: MCP Supabase execute_sql · verificación de equivalencia marítima por
-- checksum pre/post · rollback = vieja_def.sql
-- ═══════════════════════════════════════════════════════════════════════════
