-- ROLLBACK T3·1 — restaura la definición previa EXACTA (pg_get_viewdef pre-apply)
-- Nota: la columna inicia_transito desaparece → DROP VIEW + CREATE (no OR REPLACE)
DROP VIEW public.v_operacion_estado;
CREATE VIEW public.v_operacion_estado AS
 WITH universe AS (
         SELECT seguimiento_ordenes.order_number
           FROM seguimiento_ordenes
        UNION
         SELECT mailing_orders.order_number
           FROM mailing_orders
          WHERE mailing_orders.order_number ~ '^[1-9]\d{6,11}$'::text
        UNION
         SELECT v_bl_controls_latest.order_number
           FROM v_bl_controls_latest
          WHERE v_bl_controls_latest.order_number ~ '^[1-9]\d{6,11}$'::text
        UNION
         SELECT certificados_origen.orden
...
-- re-aplicar grants/candados de la vista (los de la creación original) + reload:
