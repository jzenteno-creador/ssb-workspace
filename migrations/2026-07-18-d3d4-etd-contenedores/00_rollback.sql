-- ═══════════════════════════════════════════════════════════════════════════
-- ROLLBACK — D3+D4: etd / roleo_to_etd / contenedores en v_operacion_estado
-- ═══════════════════════════════════════════════════════════════════════════
-- QUÉ REVIERTE: la migración 01_migration.sql (agrega 3 columnas al final del
-- SELECT de v_operacion_estado: etd, roleo_to_etd, contenedores). Este archivo
-- restaura la vista a su definición EXACTA previa a esa migración.
--
-- CUÁNDO USARLO: si tras aplicar 01_migration.sql aparece cualquier regresión
-- en un consumer de v_operacion_estado (Seguimiento marítimo/terrestre,
-- badges, etc.) — correr este archivo completo vía el mismo canal usado para
-- aplicar (apply_migration / execute_sql de a piezas si corta conexión).
--
-- PROVENANCIA DE ESTA DEFINICIÓN (IMPORTANTE — leer antes de asumir nada):
-- NO es una copia de migrations/2026-07-17-t3-deadline-por-modo/nueva_def.sql
-- (ese archivo quedó STALE — ver README.md de esta carpeta, hallazgo del
-- paso 1 del prep). Este CREATE OR REPLACE es la definición VIVA verificada
-- el 2026-07-18 vía `pg_get_viewdef('public.v_operacion_estado'::regclass,
-- true)` contra la DB de prod (xkppkzfxgtfsmfooozsm), idéntica byte-a-byte
-- (salvo el encabezado CREATE OR REPLACE) a
-- migrations/2026-07-17-r2g-co-por-origen/viewdef_post.sql. Incluye ya:
--   · la lógica de co_requerimiento por ORIGEN (orden_productos.origen
--     ~~ 'ARGENTIN%' ⇒ requerido) de R2G, que sustituyó la regla PERU-only
--     de T3 en algún punto entre T3 y este prep.
--   · la columna doc_pl (packing list) agregada por T5-doc-pl.
-- Es decir: revertir con este archivo deja la vista EXACTAMENTE como estaba
-- ANTES de aplicar 01_migration.sql — con R2G y doc_pl intactos, SIN romper
-- ninguna feature en prod.
--
-- INVARIANTE: CREATE OR REPLACE VIEW sobre el mismo OID preserva grants/RLS
-- existentes — este rollback NO necesita recrear la vista desde cero. El
-- REVOKE de abajo es defensivo (mismo patrón que 2026-07-17-t3.../rollback.sql),
-- no una corrección de un estado roto.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- re-aplicar grants/candados de la vista (defensivo, mismo patrón que
-- 2026-07-17-t3-deadline-por-modo/rollback.sql) + reload:
