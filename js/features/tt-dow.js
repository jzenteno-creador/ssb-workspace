/* === TARIFAS TERRESTRES DOW (js/features/tt-dow.js — ES Module, balde 2) ===
   ...
   scope de módulo reemplaza al wrapper). EL MÁS GRANDE de la extracción
   (1.920 líneas). CRÍTICO: el dirty-guard `window.__ttHasPendingChanges`
   (seteado desde `_hasPendingChanges`) es lo que nav.js consulta ANTES de
   cambiar de tab — si esa asignación no viaja EXACTA, se pierde el aviso
   de "cambios sin guardar" y hay pérdida de datos silenciosa. Preservada
   VERBATIM, sin tocar.
   ...
   más 5 handlers inline generados dentro de este mismo bloque (HTML de
   filas dinámicas), que viajan con él. Delegación propia por escucha
   'click' delegada sobre `data-action` con prefijo `tt-*`, más 2×'change'
   y 1×'keydown' (Enter con preventDefault) y 1×'beforeunload' en window —
   5 listeners en total, todos top-level, corren en module-eval (post-DOM
   completo), sin pérdida funcional. ~29 `window.X=` exports preservados VERBATIM (contrato
   con el markup y con nav.js) — CERO `export` (el contrato sigue siendo
   window). Consume de clásicos SIEMPRE pelados (regla dura CLAUDE.md,
   nunca window.X): `esc` (helpers.js), `ssbToast`/`ssbAlert`/`ssbConfirm`
   ...
   y sobre `_doApplyTTFilterImpl`, en una const local) — helpers.js sigue
   clásico, en module-eval resuelven igual; PROHIBIDO window.debounce.
  ...
  function _renderChips(kind){
    const wrap   = document.getElementById('tt-' + kind + '-input');
    const search = document.getElementById('tt-' + kind + '-search');
  ...
  function _renderDropList(kind){
    const drop = document.getElementById('tt-' + kind + '-drop');
    if(!drop) return;
    const sel = _getSetByKind(kind);
    if(!sel) return;
    const search = (document.getElementById('tt-' + kind + '-search')?.value || '').toUpperCase();
    ...
    drop.innerHTML = items.map((v, i) => {
      ...
      return `<div data-action="tt-multi-tog" data-kind="${esc(kind)}" data-value="${esc(v)}" role="option" aria-selected="${on}" style="display:flex;align-items:center;gap:8px;padding:7px 12px;cursor:pointer;${sep};font-size:13px;color:var(--text);user-select:none">
  ...
  function _renderUI(kind){ _renderChips(kind); _renderDropList(kind); }

  function _highlightInputOpen(kind, on){
    const inp = document.getElementById('tt-' + kind + '-input');
    if(!inp) return;
    if(on){
  ...
  function _closeDrop(kind){
    const drop = document.getElementById('tt-' + kind + '-drop');
    if(drop) drop.classList.remove('open');
    _highlightInputOpen(kind, false);
    if(_ttOpenKind === kind) _ttOpenKind = null;
  }

  window.openTTDrop = function(kind){
    if(_ttOpenKind && _ttOpenKind !== kind) _closeDrop(_ttOpenKind);
    const drop = document.getElementById('tt-' + kind + '-drop');
    if(!drop) return;
    _renderDropList(kind);
  ...
  window.filterTTDrop = function(kind){
    if(_ttOpenKind !== kind) openTTDrop(kind);
    else _renderDropList(kind);
  };

  window.onTTDropKey = function(e, kind){
    if(e.key === 'Escape'){
      _closeDrop(kind);
      const inp = document.getElementById('tt-' + kind + '-search');
      if(inp) inp.blur();
    }
  };
  ...
  function _togFilter(kind, value){
    ...
    // Refrescar los 5: cambiar un filtro cambia las opciones leave-one-out de los otros.
    for(const kk of _TT_KINDS) _renderUI(kk);
    _doApplyTTFilter();
  }
  ...
  function _renderEditorPill(){
    ...
    if(!pill || !nm) return;
    const name = _loadEditor();
    if(name){ nm.textContent = name; pill.style.display = ''; }
    else    { pill.style.display = 'none'; }
  }

  // Asegurar que haya editor name antes de cualquier acción de edición.
  function _ensureEditor(callback){
    if(_loadEditor()){ if(callback) callback(); return; }
    _showModal({
  ...
  function _hasPendingChanges(){
    return Object.keys(_ttPendingChanges).length > 0 || _ttPendingNew.length > 0;
  }
  ...
  function _populateDatalists(){
    const sets = { carriers:new Set(), departures:new Set(), destinations:new Set(), paises:new Set(), aduanas:new Set() };
    for(const r of (_ttData || [])){
      if(r.carrier)      sets.carriers.add(r.carrier);
      if(r.departure)    sets.departures.add(r.departure);
      if(r.destination)  sets.destinations.add(r.destination);
      if(r.pais_destino) sets.paises.add(r.pais_destino);
      if(r.customs_exit) sets.aduanas.add(r.customs_exit);
    }
    for(const c of (_ttCarriers || [])) sets.carriers.add(c.nombre);
    const dlMap = {
      'tt-dl-carriers':     [...sets.carriers].sort((a,b)=>a.localeCompare(b,'es')),
      'tt-dl-departures':   [...sets.departures].sort((a,b)=>a.localeCompare(b,'es')),
      'tt-dl-destinations': [...sets.destinations].sort((a,b)=>a.localeCompare(b,'es')),
      'tt-dl-paises':       [...sets.paises].sort((a,b)=>a.localeCompare(b,'es')),
      'tt-dl-aduanas':      [...sets.aduanas].sort((a,b)=>a.localeCompare(b,'es'))
    };
    for(const [id, vals] of Object.entries(dlMap)){
      const dl = document.getElementById(id);
      if(dl) dl.innerHTML = vals.map(v => `<option value="${esc(v)}">`).join('');
    }
  }

  function _renderEditableRow(r, id, tempId, rowClass){
    const COLS = '1.2fr 1.4fr 1.4fr 1.1fr 1.4fr .9fr .55fr';
    ...
    return `<div class="sched-row-wrap ${cls}" ${idAttr} ${tidAttr}>
      <div class="sched-main-row" style="grid-template-columns:${COLS};padding:6px 12px;gap:6px">
        ...
        <input type="text" class="tt-cell-edit" data-field="destination"  value="${esc(r.destination||'')}" placeholder="Destino" autocomplete="off">
  ...
  function _renderEdit(){
    ...
    merged.sort(_editRowComparator);
    ...
    if(ct) ct.textContent = (_ttData.length + _ttPendingNew.length) + ' filas en total';
    _updatePendingPill();
  }
  ...
  window.discardTTAll = function(){
    ...
    _showModal({
      ...
      subtitle: 'Vas a perder ' + n + ' cambio(s) sin guardar. Esta acción no se puede deshacer.',
      label: '',
      multiline: false,
      placeholder: '',
    ...
    _ensureEditor(() => {
      ...
      _showModal({
        ...
        onConfirm: async (motivo) => {
          ...
          // Limpiar pending y recargar desde DB.
          _ttPendingChanges = {};
          _ttPendingNew = [];
          _ttPendingNewSeq = 0;
          _ttData = null; _ttLastFetch = 0;
  ...
  function _buildFilterRow(){
    const row = document.getElementById('tt-filter-row');
    if(!row) return;
    const kinds = [
    ...
    row.innerHTML = kinds.map(k => `
      <div class="ac-wrap" id="tt-${k.id}-wrap" style="flex:1 1 200px;min-width:180px;max-width:280px">
        <div class="ac-in" id="tt-${k.id}-input" tabindex="0" role="combobox" aria-haspopup="listbox" aria-expanded="false" aria-label="Filtro ${esc(k.label)}"
             style="display:flex;flex-wrap:wrap;align-items:center;gap:4px;min-height:34px;padding:4px 8px;cursor:text;width:auto"
             onclick="openTTDrop('${k.id}')">
          <input type="text" id="tt-${k.id}-search" placeholder="${esc(k.label)}…" autocomplete="off"
                 ...
                 oninput="filterTTDrop('${k.id}')" onfocus="openTTDrop('${k.id}')" onkeydown="onTTDropKey(event,'${k.id}')">
        </div>
        <div class="ac-drop" id="tt-${k.id}-drop" role="listbox" aria-label="Lista de ${esc(k.label)}" style="left:0;right:auto;min-width:240px;max-height:240px"></div>
      </div>`).join('') +
    ...
    for(const k of kinds) _renderUI(k.id);
  }
  ...
  window.switchTTMode = async function(mode){
    ...
    // FIX (auditoría Tanda 1): avisar al salir del modo edición con cambios sin guardar.
  ...
  document.addEventListener('click', (e) => {
    ...
    // Outside-click: cerrar drops abiertos
    if(_ttOpenKind){
      const wrap = document.getElementById('tt-' + _ttOpenKind + '-wrap');
