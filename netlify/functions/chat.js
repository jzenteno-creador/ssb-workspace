// Netlify Function — Agente SSB con text-to-SQL contra MySQL
// Dos llamadas a Claude: 1) generar SQL  2) responder con los resultados
// Credenciales server-side (nunca expuestas al browser)
...
function validateSql(sql) {
  ...
  // Verificar que solo referencia tablas permitidas
  ...
  for (const m of fromMatches) {
    ...
    if (!ALLOWED_TABLES.includes(table) && table.length > 2) {
      throw new Error(`Tabla "${table}" no permitida. Solo: ${ALLOWED_TABLES.join(', ')}.`);
    }
  }
  // Forzar LIMIT si no tiene
...
export async function handler(event) {
  ...
  try {
    ...
    try {
      ...
      // Si falla la validación, responder sin datos
