...
- **Motor de mails (Mailing WF `kh6TORgRg9R1Shj1`, pin vivo `4ba78653` al 24-07):** TEST_MODE ABIERTO a todo usuario logueado (candado por usuario retirado — decisión de negocio de John, NO re-agregar). Robustez en prod: timeout Gmail 30s + retry de descarga 3× (incidente 504/ZIP 23-07). **DEUDA:** el envío es SILENCIOSO si un adjunto esperado no baja (hoy lo tapa el retry) — arreglar; y el timeout estructural pide desacople async (la request no debe esperar el envío colgada). Detalle en `docs/PENDIENTES.md`. Los `--apply` de los harnesses Iron Law (`scripts/rediseno-cbl/*/put_*.py`) los corre Claude con OK verbal de John — path ABSOLUTO, sin `cd &&` (permiso en `.claude/settings.local.json::autoMode.allow`).
