...
- **DECISIÓN CERRADA (John, 17-07): el control AVISA/ALERTA para que se controle, pero NO BLOQUEA envíos.**
  Implementado y verificado: persiste (`controles_factura_pe`) + expone (`response.control_fcpe` del
  resolver de mailing) + alerta visible (warning "⚠ Control Factura ↔ Permiso: REVISAR" en el detalle de
  Mailing) — jamás entra en `block_reasons` ni viaja en el mail al cliente. Los REVISAR son revisión
  operativa de John, no acción automática.
...
## SEGUNDA RONDA (R2) — 2026-07-17 · feedback de John en prod + pedidos nuevos

> Primera instancia (T0–T8): EJECUTADA Y APROBADA — queda arriba como HECHO.
> Esta ronda usa códigos R2·x. Método idéntico (EXPLORE → PLAN → GO donde se
> indica → IMPLEMENT → VERIFY). Regla de la ronda: por cada cosa tocada, un
> SMOKE CONCRETO para John (formato "andá a X, hacé Y, tenés que ver Z").

| Código | Ítem | Estado |
|---|---|---|
| R2·0 | EXPLORE estado real (extracción factura / orden_productos / clasificador / factibilidad regla CO) | ✅ reportado — la factura trae ítem/GMID/ORIGEN por línea; el clasificador no extraía; `seguimiento_co_config` ya es dimensional con la regla Perú cargada |
| R2·A | Rediseño lógica CO — **GO RECIBIDO con definiciones de John**: base = origen argentino ⇒ requiere · excepciones por PAÍS (Perú) y por CLIENTE (**TdF = Río Chico y Plásticos de la Isla Grande/Pixa** — la factura no distingue TdF, la señal confiable es el cliente) · override por orden SE MANTIENE como válvula secundaria · "por destino" = PAÍS (no puerto) | ✅ GO → implementado en R2·G |
| R2·B | **🛑 GO GATE v2** — mockup Seguimiento: el v1 fue **RECHAZADO** (dejó el toggle interno y no hizo los dos ítems del rail) → **REHECHO con FOCO MARÍTIMO** (John lo necesita operativo YA; terrestre = fase 2 con el mismo patrón): dos ítems del rail sin toggle, full-width, triangulación discreta, desplegable producto+contenedores, "Despacho de planta", control para comparar columnas CO/Progreso | Mockup NUEVO servido: `http://localhost:8899/docs/mockups/mockup_r2_seguimiento_maritimo_v2.html` — **espera GO visual**; John itera sobre este archivo |
| R2·C | Extracción de factura POR ORDEN: ítem + GMID + cantidad + nombre + ORIGEN → `orden_productos` (+ `origen`, `item_nos`) | ✅ CBL E2E REAL (reproceso → origen='Argentina', item_nos=[1,2,3]; pin `051731bd`) + rama de 7 nodos en el clasificador Gmail→Drive (extrae AL CAPTURAR; prompt/schema clonados del CBL; pin `b8d997d6`) — E2E del clasificador con la próxima factura real |
| R2·D | Idioma del mail por país destino: Brasil→pt · LATAM hispana→es · USA/resto→en | ✅ LIVE (pin mailing `cf496aef`): 118833340→pt · 118979709→es · asunción marcada: no-LATAM/no-USA queda en inglés |
...
| **R2·J** | Mockup Seguimiento TERRESTRE (fase 2 — diferencias del modo: **inicia tránsito** en vez de zarpe, **CRT en vez de BL**, **sin Control BL**, sección TRANSPORTE CRT/MIC, PE no-aplica para STO) | 🛑 GO GATE — servido en `http://localhost:8899/docs/mockups/mockup_r2_seguimiento_terrestre.html`, diferencias marcadas en ámbar |
...
| 2026-07-17 | **T5 (D.2+D.4) APLICADA**: `documentos_orden` (827 docs backfill Drive, 190/190 órdenes) + PUT Gmail→Drive 41→36 nodos (captura + fix OCR pendiente de credencial P·7) + chips reales en Seguimiento. **A.2 CERRADO de raíz**: resend verificado (exec 33411) + created_at refresca (A2-FIX) + front "Reprocesando…" con poll (deploy `4069d52`) | Root-fix B.8; NO-OK de John en A.2 diagnosticado contra ejecuciones reales |
| 2026-07-17 | **T6 (G.2 mail) APLICADA — TEST_MODE SIGUE ON**: columnas mail (etd/eta/incoterm/freight_term/shipment_no, backfill 84/84 + wiring CBL pin `ac1bf25d`) · template "Slim & Structured" EN de la guía (PUT 3 targets) · FREE DAYS vía `v_orden_freetime` con regla hub P·5 · checklist con fuente `documentos_orden` (+nodo GET, "(to follow)") · banderas emoji vía puertos→paises. Pins mailing `bce090d2`→`f997deff`→`03d7b7a0` | Smokes técnicos: preview live 13/13 + ejecución 33413 inspeccionada; flip a real = STOP T6·5 (solo John) |
| 2026-07-17 | **T7 (D.3) APLICADA**: diseño en `docs/plans/DESIGN_D3_factura_vs_permiso_2026-07-17.md` · `orden_productos` + `controles_factura_pe` + backfill (89 productos/86 órdenes; 88 controles: 72 OK · **15 REVISAR genuinos — familia CIP, seguro FC↔PE desparejo** · 1 NO_APLICA) · rama aditiva CBL (69→73, pin `1cebb413`) verificada con reproceso real · bloque PRODUCT en el mail (pins mailing →`88955fa2`→`c5920781`, bugfix dedup ×4 detectado y corregido en el mismo ciclo) | El control NO gatea envíos (persistir ≠ gatear — gates nuevos = decisión John aparte) |
| 2026-07-17 (2ª) | **Feedback smokes John**: mail T6 APROBADO · A.2-resend APROBADO · A.2-front AJUSTE aplicado (feedback inmediato al click: "Enviando…"+spinner+disabled pre-await, restore ante error; E2E 12/12) · **decisión FC-PE CERRADA: alerta, NO bloqueo** — implementada (nodo `GET controles_factura_pe`, `response.control_fcpe`, warning en Mailing; pin `2d61baed`) y verificada live (118762005) · chips T5·4 y D.3 verificados por Claude con evidencia (John no smokea esos a mano) · 3 mockups servidos en `localhost:8899/docs/mockups/` para aprobación | Smokes John 17-07 + autonomía sin gate visual |
| 2026-07-17 (R2-4ª) | **GO al mockup marítimo → R2·F EN PROD** (deploy `7abe347`): DOS ítems del rail (Marítimo `#i-package` / Terrestre `#i-route`) vía `segGo()`, toggle interno MUERTO, badge por solapa, tabla full-width (max-width none), 12 columnas sin CO/Progreso, Docs = badge n/m, triangulación pill, desplegable ▸ con 3 secciones (PRODUCTO ítem/GMID/cantidad/ORIGEN · CONTENEDORES del control lazy · DOCUMENTOS está/falta con archivo real + link Drive + estado Control BL), botón/modal "Despacho de planta" con alta heredando el modo (headless 18/18, canario 2). Terrestre conserva layout previo hasta fase 2 · **R2·I EN PROD** (deploy `830b515`): dirección del BA por destinatario — hallazgo: la extracción YA existía, era plomería (CBL `9f69b166` + backfill 84/84 + resolver `943bbc15` + front) + país junto al puerto · **R2·J**: mockup TERRESTRE servido (GO gate — diferencias del modo en ámbar: inicia tránsito, CRT por BL, sin Control BL) | GO de John al marítimo + pedidos Mailing; terrestre espera su GO |
| 2026-07-17 (R2-3ª) | **Mockup marítimo v2 gustó — agregado pedido**: sección **DOCUMENTOS PARA EL ENVÍO** en el desplegable (presencia está/falta por tipo con archivo real de `documentos_orden` + estado del Control BL en el encabezado; **SIN "aprobado" por documento** — decisión John: futuro). Mockup actualizado en la misma URL, sigue como GO gate. **+R2·H registrado como FUTURO** (no ejecutar): control CO vs Factura — FOB o valor total según destino, criterio CHILE ≠ MERCOSUR, patrón D.3 aplicado al CO; arrastra extracción del CO (ZIP/PDF) | Iteración de John sobre el mockup + definición nueva a futuro |
| 2026-07-17 (R2-2ª) | **Gates resueltos**: R2·D smoke APROBADO (mail portugués en prod; el 501/"Preview falló" que vio John era el MOCKUP local sin backend — prod sano) · **R2·A GO con definiciones** (TdF por CLIENTE: Río Chico + Plásticos de la Isla Grande/Pixa · override por orden = válvula secundaria · destino=PAÍS) → **R2·G EN PROD**: seed excepciones TdF + vista con regla ORIGEN (byte-verificada, hardcode Perú eliminado) + Admin re-orientada a excepciones (deploy `45fe7b8`) · **R2·B v1 RECHAZADO** (dejó el toggle interno, no hizo los 2 ítems del rail) → **mockup v2 REHECHO con FOCO MARÍTIMO** (`mockup_r2_seguimiento_maritimo_v2.html`) — terrestre = fase 2 con el mismo patrón | Correcciones de John sobre los 2 gates; marítimo operativo HOY es la prioridad |
| 2026-07-17 (R2) | **SEGUNDA RONDA abierta** (feedback John en prod, info en crudo): sección R2 nueva con códigos propios. EXPLORE R2·0 reportado · **R2·C APLICADA** (extracción factura por orden: CBL pin `051731bd` E2E real + clasificador pin `b8d997d6` — ítem/GMID/ORIGEN a orden_productos) · **R2·D APLICADA** (mail pt/es/en por destino, LIVE ×3) · **R2·E APLICADA** (bloque Log-In textual, LIVE; P·6 REDEFINIDO: no era la tabla de Naara, es bloque fijo por naviera — Maersk/Hapag diferidos) · R2·A (rediseño CO) y R2·B (mockup Seguimiento v2) = GO GATES esperando a John · pins mailing `bce090d2→…→cf496aef` | Pedidos R2 de John + decisiones cerradas (dos solapas rail · botón "Despacho de planta" · desplegable con producto+contenedores · CO por factura/origen con excepciones administrables · Log-In textual solo) |
| 2026-07-17 (3ª) | **LOS 3 MOCKUPS APROBADOS por John → 3 FRONTS IMPLEMENTADOS Y EN PROD** (merge `7d2ace9`): **B.1-fix** segmentado Transporte del lote + override M/T por fila + conflicto espejo (headless 7/7) · **T3·3 Seguimiento por modo** — sub-solapas B.7 (select Transporte retirado), plazo terrestre B.2 con día hábil anotado (`terrBucket` propio, helpers intactos), **banderas B.3 = flagcdn** (directiva John pisa el emoji del mockup; mapa paises ES+EN client-side), Ship-to/Sold-to + triangulación B.4, export Excel B.6, leyenda por modo; **B.8-soft NO implementado** (regresaría el root-fix T5) · **T8/E.1 Admin CO** — solapa nueva `admin-co` (módulo ES + rail `#i-sliders` + panel, `.ssb-admin-only` + guard isAdmin), tabla con filtro default "sin definir", editor AUTO/SÍ/NO con motivo obligatorio, audit trail (lectura directa a `seguimiento_ordenes`), 3 cards futuras con candado; alerta "¿Lleva CO?" deep-linkea; **`set_requiere_co` promovida a ADMIN_ACTIONS** (decisión propia: cero consumidores previos, la solapa es admin — reversible 1 línea) · **BUG BANDERAS MAIL corregido**: emoji → img flagcdn con alt=nombre de país (bloqueo de imágenes degrada a "Brazil", jamás "BR"); pin mailing **`a624b3d5`**, verificado live + headless en ambos escenarios | GO visual de John a los 3 mockups + bug report banderas (Windows degrada emoji al código crudo) |
